...
def get_compass_eval_fn(
    ...
    def eval_fn(params: FrozenDict, key: PRNGKey, init_act_state: ActorState) -> Metrics:
        ...
        def _env_step(
            eval_state_latent: Tuple[_EvalEnvStepState, Array], _: Any
        ) -> Tuple[_EvalEnvStepState, TimeStep]:
            """Performs a single environment step"""
            eval_state, latent = eval_state_latent
            env_state, ts, key, actor_state = eval_state

            key, act_key = jax.random.split(key)

            latent_agents_view = jnp.concatenate([ts.observation.agents_view, latent], axis=-1)
            ts.observation = ts.observation._replace(agents_view=latent_agents_view)

            action, actor_state = act_fn(params, ts, act_key, actor_state)
            env_state, ts = jax.vmap(jax.vmap(env.step))(env_state, action)

            eval_state = (env_state, ts, key, actor_state)

            return (eval_state, latent), ts
        ...
        def _episode(key: PRNGKey, _: Any) -> Tuple[PRNGKey, Metrics]:
            ...
            # duplicate reset_keys 4 times to make shape (n_vmapped_envs, num_latent_per_env, 2)
            reset_keys = jnp.stack([reset_keys] * config.arch.num_latents_per_env, axis=1)
            ...
            env_state, ts = jax.vmap(jax.vmap(env.reset))(reset_keys)

            step_state = env_state, ts, key, init_act_state
            _, timesteps = jax.lax.scan(
                _env_step, (step_state, latent), jnp.arange(env.time_limit + 1)
            )
            ...
            # TODO: carry on here to take on the best latent.
            done_idx = jnp.argmax(
                timesteps.last(), axis=0
            )  # shape: (n_vmapped_envs, num_latent_per_env)
            env_idx = jnp.arange(n_vmapped_envs)[:, None]  # Add dimension for broadcasting
            latent_idx = jnp.arange(config.arch.num_latents_per_env)[
                None, :
            ]  # Add dimension for broadcasting

            metrics = tree.map(lambda m: m[done_idx, env_idx, latent_idx], metrics)
            del metrics["is_terminal_step"]  # uneeded for logging

            # Select best metric value over the latents.
            metrics = tree.map(lambda m: jnp.max(m, axis=1), metrics)
