...
# TODO: Rollout over full episodes instead. Or select the max value in a better way to determine
# the best latent to train on.

# TODO: Check that env keys are the same over all latents.
...
def get_learner_fn(
    ...
    def _update_step(learner_state: LearnerState, _: Any) -> Tuple[LearnerState, Tuple]:
        ...
        def _env_step(
            learner_state_latent: Tuple[LearnerState, chex.Array], _: Any
            ...
            latent_agents_view = jnp.concatenate(
                [last_timestep.observation.agents_view, latent], axis=-1
            )
            last_timestep.observation = last_timestep.observation._replace(
                agents_view=latent_agents_view
            )
        ...
        latent = latent[:, :, jnp.newaxis, :].repeat(config.system.num_agents, axis=2)
        ...
        # CALCULATE ADVANTAGE
        params, opt_states, key, env_state, last_timestep = learner_state
        latent_last_ts_agents_view = jnp.concatenate(
            [last_timestep.observation.agents_view, latent], axis=-1
        )
        last_val_obs = last_timestep.observation._replace(agents_view=latent_last_ts_agents_view)
        last_val = critic_apply_fn(params.critic_params, last_val_obs)

        # Select the best latent for each environment, note we assume agents share the same reward
        # hence the zero slice on the agent dimension.
        # TODO: mask the rollout after the first done.
        rollout_returns = jax.tree.map(lambda x: x.sum(axis=0), traj_batch.reward)[:, :, 0]
        ...
        # The key fix: Use advanced indexing by adding arange for the environment dimension
        env_idx = jnp.arange(rollout_returns.shape[0])
        traj_batch = tree.map(lambda x: x[:, env_idx, best_latent_idxs], traj_batch)

        # do the same for last_val
        last_val = last_val[env_idx, best_latent_idxs]
...
def learner_setup(
    ...
    init_latent = jnp.ones((config.system.num_agents, config.arch.compass_latent_dim))
    latent_agents_view = jnp.concatenate([obs.agents_view, init_latent], axis=-1)
    obs = obs._replace(agents_view=latent_agents_view)
    ...
    # Reshape env_keys to match the desired structure before splitting for latents
    env_keys = jnp.stack(env_keys).reshape(
        n_devices, config.system.update_batch_size, config.arch.num_envs, -1
    )

    # Split keys for latents - same key used for each latent in an env
    env_latent_keys = jax.vmap(
        jax.vmap(jax.vmap(lambda k: jnp.repeat(k[None], config.arch.num_latents_per_env, axis=0)))
    )(env_keys)

    # Flatten for vmap over all environments and latents
    flat_env_latent_keys = env_latent_keys.reshape(-1, env_latent_keys.shape[-1])

    # Initialize environments with vmap
    env_states, timesteps = jax.vmap(env.reset, in_axes=(0))(flat_env_latent_keys)

    # Reshape function now includes the latent dimension
    ...
    # Reshape both env_states and timesteps
