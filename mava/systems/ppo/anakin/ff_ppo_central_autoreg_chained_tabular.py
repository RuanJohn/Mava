...
from mava.evaluator import get_eval_fn, make_autoreg_ff_tabular_eval_act_fn
from mava.networks import FeedForwardActor as Actor
from mava.networks.heads import AutoregressiveChainedTabularPolicy
...
def get_learner_fn(
    ...
    def _update_step(learner_state: LearnerState, _: Any) -> Tuple[LearnerState, Tuple]:
        ...
        def _env_step(learner_state: LearnerState, _: Any) -> Tuple[LearnerState, PPOTransition]:
            ...
            action, log_prob = actor_apply_fn(
                params.actor_params, policy_key, batch_size, method="act_fn"
            )
        ...
        # All agents have the same reward, so we can use the first agent's reward.
        advantages, targets = traj_batch.reward[:, :, 0] - jnp.array(config.env.mean_payoff), None
        ...
        def _update_epoch(update_state: Tuple, _: Any) -> Tuple:
            ...
            def _update_minibatch(train_state: Tuple, batch_info: Tuple) -> Tuple:
                ...
                def _actor_loss_fn(
                    ...
                    log_prob = actor_apply_fn(
                        actor_params, key, batch_size, traj_batch.action, method="train_fn"
                    )
...
def learner_setup(
    ...
    actor_network = AutoregressiveChainedTabularPolicy(
        num_agents=config.system.num_agents, num_actions=num_actions
    )
    ...
    actor_params = actor_network.init(actor_net_key, actor_net_key, 1, method="act_fn")
...
def run_experiment(_config: DictConfig) -> float:
    """Runs experiment."""
    _config.logger.system_name = "ff_ppo_central_autoreg_chained_tabular"
    ...
    eval_act_fn = make_autoreg_ff_tabular_eval_act_fn(actor_network.apply, config)
...
@hydra.main(
    ...
    config_name="ff_ppo_central_autoreg_chained_tabular.yaml",
    version_base="1.2",
)
def hydra_entry_point(cfg: DictConfig) -> float:
    ...
    print(
        f"{Fore.CYAN}{Style.BRIGHT}Central PPO Autoregressive Chained Tabular experiment completed{Style.RESET_ALL}"
    )
    return eval_performance
